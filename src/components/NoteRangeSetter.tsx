...
import {
  selectBtnRangeVisible,
  selectCurTimeNoteId,
} from "../store/appstore/selector";
...
export default function RangeSlider() {
  ...
  const inValue =
    inoutRange?.timeIn !== undefined ? inoutRange?.timeIn * lengthOfVid : 25;
  const outValue =
    inoutRange?.timeOut !== undefined ? inoutRange.timeOut * lengthOfVid : 75;
        ...
        {show ? (
          <div className={classes.root}>
            <Slider
              min={0}
              max={lengthOfVid}
              style={{ position: "relative", width: `${inValue + outValue}` }}
              value={value}
